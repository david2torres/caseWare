...
/**
 * Demo only: prints what the server would answer for a few engagements. Not
 * part of the logic.
 */
public class DemoMain {
    public static void main(String[] args) {
        ...
        List<EngagementTemplateBaseline> index = List.of(
                EngagementTemplateBaseline.indexed("ENG-1005", "Cedar Peak Services 2026", "REVIEW-CA", 8,
                        SampleData.INDEXED_AT),
                EngagementTemplateBaseline.indexed("ENG-1006", "Westmount Consulting 2026", "REVIEW-CA", 7,
                        SampleData.INDEXED_AT),
                EngagementTemplateBaseline.indexed("ENG-1007", "Bluewater Hospitality 2026", "REVIEW-CA", 6,
                        SampleData.INDEXED_AT));

        // 2. Logic wired with in-memory data (in real life: catalog table and diff
        // cache)
        ...
        var summaries = new PendingUpdateSummaryService(diffs, new ChangeDescriber(TemplateVocabulary.NONE),
                Clock.systemUTC());
            ...
            if (state.status() == UpdateStatus.UPDATE_AVAILABLE) {
                ...
                for (ChangeItem item : summary.items()) {
                    System.out.printf("   [%s] %s: %s  (before=%s, after=%s, changed in %s)%n",
                            item.kind(), item.area(), item.title(), item.before(), item.after(),
                            item.changedInVersions());
                }
            }
        }
    }
